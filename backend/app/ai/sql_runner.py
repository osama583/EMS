...
def rows_to_document(rows: list[dict], *, sql: str, scope=None) -> str:
    ...
    lines = [
        ...
        " AND DO NOT HEDGE ABOUT IT EITHER, which is the same mistake wearing a politer face."
        " Asked for free events, twenty rows came back - every one of them free, because that is"
        " what the query selected on - and the reply was 'none of the event details specify whether"
        " they are free'. The filter already ran. If rows are present, they satisfy the question:"
        " answer it plainly ('here are the free ones coming up') rather than reporting that you"
        " cannot tell."
