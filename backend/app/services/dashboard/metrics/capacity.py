...
def water_commitment(cur, scope: Scope, horizon: int | None = None) -> list[dict[str, Any]]:
    """M30, water variant - the one genuine inventory constraint F&B owns.

    Every other F&B resource is a cafeteria's kitchen, which is not a stock
    level and cannot be forecast the same way.
    """
    rows = fetch_all(
        cur,
        """
        SELECT o.water_normal_option_id AS option_id,
               o.label AS label,
               o.available_stock AS available,
               coalesce(sum(w.quantity), 0) AS committed,
               min(w."date") FILTER (WHERE w."date" >= %(today)s) AS first_date
          FROM water_normal_options o
          LEFT JOIN request_mineral_water w
                 ON w.option_id = o.water_normal_option_id
                AND w."date" >= %(today)s
                AND w."date" < %(today)s::date + %(horizon)s
          LEFT JOIN request r ON r.request_id = w.request_id
         WHERE o.active AND o.archived_at IS NULL
           AND (w.request_mineral_water_id IS NULL OR r.status <> ALL(%(non_committed)s))
      GROUP BY 1, 2, 3
      ORDER BY 2
        """,
        _horizon_params(scope, horizon),
    ...
    out = []
    for r in rows:
        committed = num(r["committed"]) or 0.0
        available = num(r["available"]) or 0.0
        out.append(
            {
                "optionId": r["option_id"],
                "label": r["label"],
                "committed": committed,
                "available": available,
                "ratio": ratio(committed, available),
...
def water_runway_days(cur, scope: Scope) -> dict[str, Any]:
    """Days until the first water option's committed volume exceeds its stock.

    Expressed as days rather than a ratio because that is the form the decision
    takes: a manager reorders on a lead time, not on a percentage.
    """
    rows = fetch_all(
        cur,
        """
        SELECT o.label AS label,
               ...
               w."date" AS day,
               sum(w.quantity) OVER (
                   PARTITION BY o.water_normal_option_id ORDER BY w."date"
               ) AS running
          FROM request_mineral_water w
          JOIN water_normal_options o ON o.water_normal_option_id = w.option_id
    ...
    for r in rows:
        running = num(r["running"]) or 0.0
        available = num(r["available"]) or 0.0
        if available and running > available:
            return {
                "days": (r["day"] - scope.today).days,
                "label": r["label"],
                "date": r["day"].isoformat(),
            }
    return {"days": None, "label": None, "date": None}
