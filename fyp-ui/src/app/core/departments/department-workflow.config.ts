...
// RBAC redesign: every department workflow except CFO is keyed by `unitCode`.
...
// The CFO owns NO department-review request kind: Funding/Purchase is recorded on the form but is not
// part of the approval workflow (no request_task is created for it — see
// server/services/workflow.service.js's NON_WORKFLOW_REQUIREMENTS).
export const FLAT_DEPARTMENT_WORKFLOWS: readonly FlatDepartmentWorkflowConfig[] = [
  // 'venue' joins funding as CFO-curated master data: the university's venue list is finance-governed,
  // and putting it here is what gives the Venue Management page the same ownership check, nav grant
  // and Page Visibility row as every other dropdown catalogue rather than a special case.
...
export function workflowForManager(identity: WorkflowIdentity): UnitDepartmentWorkflowConfig | FlatDepartmentWorkflowConfig | undefined {
  ...
  // Cafeteria Manager manages their OWN cafeteria's My Menu (fmb options only) — it is NOT a head-of-
  // department/head-of-school, so it needs its own branch here rather than falling into the headedUnit
  // lookup below.
...
// How many staff may be assigned to a SINGLE requested row for this request kind - mirrors the
// backend's MAX_ASSIGNEES_PER_ROW (workflow/constants.py), which is the actual enforcement; this copy
// only drives the picker's UI (single- vs multi-select), never a security decision.
...
// Which departments require staff to wait until the task's scheduled day to start it, vs.
...
// Looks up requestKinds for either a manager OR a staff-level identity of the same unit/flat role —
// since UnitDepartmentWorkflowConfig no longer distinguishes manager vs staff (both levels share the
// same unitCode -> same requestKinds), this collapses to the same lookup as requestKindsForManager for
// unit-scoped identities, plus the flat 'cfo' lookup.
export function requestKindsForRole(identity: WorkflowIdentity): readonly DepartmentRequestKind[] {
  ...
  // Cafeteria Manager owns the per-selection fmb approval/resubmit action for their own cafeteria (see
  // workflow.service.js's approveFmbSelection/resubmitFmbSelection and proposal-visibility.ts's
  // cafeteria-manager wiring) — 'fmb' here just needs to be non-empty so userIsRelatedToProposal()'s
  // routedKinds gate doesn't exclude them; the real per-selection ownership check happens in proposal-
  // visibility.ts, not here.
