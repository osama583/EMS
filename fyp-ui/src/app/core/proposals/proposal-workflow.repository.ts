...
export interface ProposalWorkflowRepository {
  ...
  deleteDraft(id: number, actorEmail: string): Observable<void>;
  ...
  // F&B's answer to a Cafeteria Manager's push-back: change the dish, quantity or cafeteria (or
  // cancel the order outright). Saving IS the re-send - see the backend's editFmbSelection().
...
export class ApiProposalWorkflowRepository implements ProposalWorkflowRepository {
  ...
  deleteDraft(id: number, actorEmail: string): Observable<void> { return this.http.delete<void>(`${this.baseUrl}/${id}`, { params: { actorEmail } }); }
  ...
  resubmitAsDepartment(id: number, department: DepartmentRequestKind, comment: string, reviewerEmail: string): Observable<ProposalReviewRecord> { return this.http.post<ProposalReviewRecord>(`${this.baseUrl}/${id}/resubmit-department`, { department, comment, reviewerEmail }); }
  resubmitFromApplicant(id: number, payload: Record<string, unknown>, actorEmail: string): Observable<ProposalReviewRecord> { return this.http.post<ProposalReviewRecord>(`${this.baseUrl}/${id}/resubmit-applicant`, { ...payload, actorEmail }); }
  saveEdits(id: number, payload: Record<string, unknown>, actorEmail: string): Observable<ProposalReviewRecord> { return this.http.post<ProposalReviewRecord>(`${this.baseUrl}/${id}/save-edits`, { ...payload, actorEmail }); }
  ...
  editFmbSelection(id: number, selectionId: number, edit: FmbSelectionEdit, reviewerEmail: string): Observable<ProposalReviewRecord> { return this.http.post<ProposalReviewRecord>(`${this.baseUrl}/${id}/fmb-selections/${selectionId}/edit`, { ...edit, reviewerEmail }); }
}
