...
import { DEPARTMENT_WORKFLOWS, DepartmentWorkflowConfig } from '../../../../core/departments/department-workflow.config';
    ...
    <div class="routing-matrix">
      <header class="routing-matrix__header">
        <div>
          ...
          <p>Read-only visualization of hardcoded department routing and role assignments.</p>
        </div>
      </header>

      <div class="routing-matrix__grid">
        @for (workflow of workflows; track workflow.managerRole) {
          <div class="routing-matrix__card">
            <h3>{{ formatRole(workflow.managerRole) }}</h3>
            
            <dl class="routing-matrix__details">
              <div>
                <dt>Staff Role</dt>
                <dd>{{ workflow.staffRole ? formatRole(workflow.staffRole) : 'None' }}</dd>
              </div>
              <div>
...
export class RoutingMatrixComponent {
  readonly workflows: readonly DepartmentWorkflowConfig[] = DEPARTMENT_WORKFLOWS;
