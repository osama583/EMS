...
export class StaffTasksComponent {
  ...
  handleAction(event: InternalRowActionEvent): void { const task = this.tasks().find((item) => item.id === event.record.id); if (!task) return; this.selected.set(task); this.pendingStatus.set(event.action.key === 'begin' ? 'preparing' : event.action.key === 'complete' ? 'completed' : null); this.modalOpen.set(true); }
  ...
  confirm(): void { const task = this.selected(), status = this.pendingStatus(); if (!task || !status) { this.closeModal(); return; } this.saving.set(true); this.error.set(''); this.service.updateStatus(task.id, status, this.auth.user()?.email ?? '').pipe(finalize(() => this.saving.set(false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: (updated) => { this.tasks.update((items) => items.map((item) => item.id === updated.id ? updated : item)); this.modalOpen.set(false); this.message.set(status === 'completed' ? `${updated.eventTitle} moved to History.` : `${this.presentation.begin} confirmed.`); }, error: () => this.error.set('The task status could not be updated.') }); }
