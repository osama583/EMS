...
export class StaffTasksComponent {
  ...
  readonly filtered = computed(() => {
    ...
    const expected = this.mode === 'history' ? 'completed' : null;
    const records = this.tasks().filter((task) => (expected ? task.status === expected : task.status !== 'completed')
  ...
  readonly filters = computed(() => [
    { key: 'status', ariaLabel: 'Filter tasks by status', value: this.statusFilter(), options: [{ value: 'all', label: 'All statuses' }, ...(this.mode === 'history' ? [{ value: 'completed', label: 'Completed' }] : [{ value: 'assigned', label: 'Assigned' }, { value: 'in-progress', label: 'In progress' }])] },
  ...
  constructor() { this.service.list(this.role, this.auth.user()!.email).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: (tasks) => { this.tasks.set(tasks); this.loading.set(false); }, error: () => { this.error.set('Tasks could not be loaded.'); this.loading.set(false); } }); }
  ...
  handleAction(event: InternalRowActionEvent): void { const task = this.tasks().find((item) => item.id === event.record.id); if (!task) return; this.selected.set(task); this.pendingStatus.set(event.action.key === 'begin' ? 'in-progress' : event.action.key === 'complete' ? 'completed' : null); this.modalOpen.set(true); }
  ...
  openRecord(record: InternalDataRecord): void { const task = this.tasks().find((item) => item.id === record.id); if (task) { this.selected.set(task); this.pendingStatus.set(null); this.modalOpen.set(true); } }
  closeModal(): void { if (!this.saving()) this.modalOpen.set(false); }
  confirm(): void { const task = this.selected(), status = this.pendingStatus(); if (!task || !status) { this.closeModal(); return; } this.saving.set(true); this.error.set(''); this.service.updateStatus(task.id, status).pipe(finalize(() => this.saving.set(false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: (updated) => { this.tasks.update((items) => items.map((item) => item.id === updated.id ? updated : item)); this.modalOpen.set(false); this.message.set(status === 'completed' ? `${updated.eventTitle} moved to History.` : `${this.presentation.begin} confirmed.`); }, error: () => this.error.set('The task status could not be updated.') }); }
  modalTitle(): string { const status = this.pendingStatus(); return status === 'in-progress' ? this.presentation.begin : status === 'completed' ? this.presentation.complete : 'Task details'; }

  private toRecord(task: StaffTask): InternalDataRecord { const tone = task.status === 'completed' ? 'success' : task.status === 'in-progress' ? 'blue' : 'warning'; return { id: task.id, actionKeys: this.mode === 'history' ? ['view'] : task.status === 'assigned' ? ['view', 'begin'] : ['view', 'complete'], cells: { event: { primary: task.eventTitle, secondary: task.eventCode }, request: { primary: task.request }, quantity: { primary: task.quantity ?? '—' }, detail: { primary: task.detail, secondary: task.detailLabel }, schedule: { primary: task.schedule, secondary: task.location }, location: { primary: task.location }, status: { primary: this.statusLabel(task.status), badge: true, tone }, actions: { primary: '' } }, mobile: { eyebrow: task.eventCode, status: this.statusLabel(task.status), title: task.eventTitle, identity: task.request, details: [{ icon: 'schedule', text: task.schedule }, { icon: 'location_on', text: task.location }, { icon: 'info', text: task.quantity ?? task.detail }] } }; }
  private statusLabel(status: StaffTaskStatus): string { return status === 'in-progress' ? 'In progress' : status[0].toUpperCase() + status.slice(1); }
}
