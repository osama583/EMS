...
// `PublishedEvent` rows ARE `request` rows that reached `completed_approved` with
// `event_visibility === 'Public'` — not a separate seed source (design spec's schema).
function isPublishedEvent(request) {
  return request.status === 'completed_approved' && request.event_visibility === 'Public';
}

function projectPublishedEvent(request) {
  const categories = db.request_categories
    .filter((rc) => rc.request_id === request.request_id)
    .map((rc) => db.event_category.find((c) => c.event_category_id === rc.category_id))
    .filter(Boolean)
    .map((c) => c.name);
